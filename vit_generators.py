...
class VitSolidTestGenerator:

    def __init__(self, iterator, random_gen, max_num, num_class, noise_level, size_in, size_out, ndiv):
        ...
        self.ndiv = ndiv
        self.psiz = size_out // ndiv
        self.ndiv2 = ndiv*ndiv
    ...
    def __next__(self):        
        ...
        while ip < npics:
            ...
            empty = np.zeros((self.imgsize,self.imgsize,1))
            masks = np.zeros(self.ndiv2)
            ...
            for j in range(h):
                ...
                empty[y,xmin:xmin+w,0] = image[j,:]
                
            xp = np.reshape(empty, [self.ndiv,self.psiz,self.ndiv,self.psiz])
            xp = np.swapaxes(xp, 1, 2)
            patch = np.reshape(xp, [self.ndiv2,-1])

            masks = labl * np.any(patch > 0, axis=1)

            for j in range(self.ndiv2):
                if (masks[j] == 0):
                    continue
                col, row = j%self.ndiv, j//self.ndiv
                iou = compute_iou(box, [col*self.psiz,row*self.psiz,self.psiz,self.psiz], self.imgsize)
                if np.max(oneh[j,1:]) < iou:
                    oneh[j] = 0
                    oneh[j,masks[j]] = iou
                    oneh[j,0] = 1 - iou

        return 1 - blanks, np.any(oneh[:,1:], axis=-1) * (np.argmax(oneh[:,1:], axis=-1) + 1)

class VitSolidTrainGenerator:

    def __init__(self, iterator, random_gen, max_num, num_class, noisefiles, noise_level, size_in, size_out, ndiv):
        self.iterator = iterator
        self.imgsize = size_out
        self.mnistsize = size_in
        self.noise = noise_level
...
class VitTestGenerator:
    ...
    def __next__(self):        
        ...
        return 1 - blanks, oneh
        
class VitTrainGenerator:
